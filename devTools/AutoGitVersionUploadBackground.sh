...
while [ $COUNTER -ge 0 ]; do
	...
	else if [ "$(git pull)" == "Already up to date." ]; then exit
		else rm bin/*.html ; .compile > /dev/null && mv bin/FC_pregmod.html "bin/FC-pregmod-$(git log -1 --format='%cd' --date='format:%F-%H-%M')-$AbrevHash.html" && mega-put bin/*.html $RemoteDir
		fi
	fi	
